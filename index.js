	...
	sock.ev.on("messages.upsert", async ({ messages, type }) => {
        ...
        if(type === "notify"){
            if(!messages[0].key.fromMe) {
                ...
                }if(!messages[0].key.fromMe && pesanMasuk === "f"){
                    await sock.sendMessage(noWa, {text: "f"},{quoted: messages[0] });
                }if(!messages[0].key.fromMe && pesanMasuk.includes("mangan opo")){
                    const makans = ['Danau Wae','Gas Tinton','Yo jelas Bakso Balelo','Terserah..','Ayoya Mie Sony Wae']
                    var makan = makans[Math.floor(Math.random()*makans.length)];
                    await sock.sendMessage(noWa, {text: makan},{quoted: messages[0] });
                }else{
